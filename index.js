var backCanvas = document.getElementById("bg_img_canvas");
var backContext = backCanvas.getContext("2d");
//调整画布大小
backCanvas.width = 2440;
backCanvas.height = 860;
//制作背景滚动的云雾
var image = new Image();
image.src = "./img/sunset333.jpg";
image.onload = function () {
    let xx = 0;
    setInterval(function () {
        backContext.clearRect(0, 0, 1280, 1024);
        backContext.save();//用来保存Canvas的状态。save之后，可以调用Canvas的平移、放缩、旋转、错切、裁剪等操作。
        //context.绘制图片(图片或画布或影片, 裁切位置x, y,  裁切宽,高,  绘制图片的位置x,y,  图片宽,高)
        backContext.drawImage(image, xx, 0, 7680, 2700, 0, 0, 7680, 800);
        // 刷新 background position.
        backContext.restore();//恢复Canvas之前保存的状态。防止save后对Canvas执行的操作对后续的绘制有影响。
        xx += 0.06;
        if (xx >= 5680) {
            xx = -0.06;
        }
    }, 1);
};



//实时获取当前复活次数并显示
function showScore(score){//显示分数
    var scoreSpan = document.getElementById('scoreSpan');
    scoreSpan .innerHTML = score;
}

//实时获取剩余秒数显示在左上角
function showSecond(second){//显示还有多少秒
    var secondSpan = document.getElementById('secondSpan');
    second<=0?second=0:second;
    secondSpan .innerHTML = second;
}


var stopInitFlag=0;
var time=20;
setInterval(function () {

    stopInitFlag++;//和秒数相同
    showSecond(time-stopInitFlag);
    if(stopInitFlag>=time){
        alert("TIME OUT,GAME OVER!");
    }

},1000)

/*小熊的添加*/
//动画计数, 熊猫的x位置, y位置
var dx =15, dy = 12, xmove = 170.33, ymove = 0;
var py = 0, x = 80, y = 300;//初始裁剪位置 初始显示位置
var px = [510.99, 681.33, 851.66, 1022, 1192.33, 1362.66, 0, 170.33, 340.66];
var i = 0;
var score = 6;//原始次数
//加载熊猫图片
var img = new Image();
img.src = "./img/sprite.png";

var img2 = new Image();
img2.src = "./img/sprite_!!!.png";

//创建画布
var canvas = document.getElementById("myCanvas");//获取元素
context = canvas.getContext("2d");//渲染环境，获得2d环境

//调整画布大小
canvas.width = 2440;
canvas.height = 580;
//初始的位置
context.drawImage(img, px[0], py,   170, 172,    80, 300,    200, 203);

/*
* canvas的裁剪图片 drawImage（）中对裁剪完成的图片绘制位置在左上角为起始位置*/
//小熊移动位置
function move(e) {
    switch (e.keyCode) {
        case 65://left A/a
            context.clearRect(0, 0, 2440, 980);
            i = i - 1;
            x = x - dx;
            if (i == -1) {//context.drawImage(图片或画布或影片, 裁切位置x, y,  裁切宽,高,  绘制图片的位置x,y,  图片宽,高)
                i = 8;
                context.drawImage(img, px[i], py, 170, 172, x, y, 200, 203);
            } else
                context.drawImage(img, px[i], py, 170, 172, x, y, 200, 203);
            break;
        case 68://right D/d
            context.clearRect(0, 0, 2440, 980);
            i++;
            x = x + dx;
            if (i == 9) {
                i = 0;
                context.drawImage(img, px[i], py + 0, 170, 172, x, y, 200, 203);
            }
            context.drawImage(img, px[i], py + 0, 170, 172, x, y, 200, 203);
            /*if(x>=)*/
            break;

        case 87://up W/w

            context.clearRect(0, 0, 2440, 980);
            y = y - dy;
            context.drawImage(img, px[i], py, 170, 172, x, y, 200, 203);
            break;

        case 83://down S/s
            //if((px+move)>0){xmove=-xmove;}
            context.clearRect(0, 0, 2440, 980);
            y = y + dy;
            context.drawImage(img, px[i], py, 170, 172, x, y, 200, 203);
            break;
    }
}


//创建画布
var cball = document.getElementById("ball");//获取元素
cball  = canvas.getContext("2d");//渲染环境，获得2d环境

//调整画布大小
cball .width = 2440;
cball .height = 565;
/*绘制运动球部分*/
var score=6;//默认6次机会

var ball={
    ballx:420,bally:550,
    ballxspeed:-2,ballyspeed:-2,

    draw:function(){
        context.save();
        context.beginPath();
        context.fillStyle="#6b43bf";
        context.arc(this.ballx,this.bally,10,0,Math.PI*2,false);
        context.fill();
        context.closePath();
        context.restore();
    },
/*判断运动的方向朝向，进行特定位置清屏操作*/
    move:function () {
       var xxx=this.ballxspeed/2;
       var yyy=this.ballyspeed/2;
       if(xxx<0&&yyy<0)
        cball.clearRect(this.ballx+10,this.bally-3,20,30);
       if(xxx>0&& yyy>0)
           cball.clearRect(this.ballx-27,this.bally-25,30,15);
       if (xxx<0&& yyy>0)
           cball.clearRect(this.ballx+10,this.bally-28,20,30);
       if(xxx>0&& yyy<0)
           cball.clearRect(this.ballx-27,this.bally+10,30,36);
        this.ballx=this.ballx+this.ballxspeed;
        this.bally=this.bally+this.ballyspeed;
    },

    //小球和熊的相撞检测
    checkCanvas:function()
    {   /*左右上下的边缘判断*/
        if(this.ballx<10||this.ballx>1480)
            this.ballxspeed=-this.ballxspeed;
        if(this.bally<10||this.bally>550)
            this.ballyspeed=-this.ballyspeed;

        /*球和小熊的位置判断*/
        if(this.bally>=(y+14) && this.bally<=(y+185) && this.ballx>=(x+40) && this.ballx<=(x+150))//小球碰撞到小熊时
        {
            context.clearRect(0, 0, 2440, 980);
            context.drawImage(img2, px[i], py, 170, 172, x, y, 200, 203);
            score--;
            showScore(score);//复活机会减一，用score记录
            if(score>=0)/*小熊的复活机会还存在*/
            {
                alert("panda have have been hitted ");
            }
            else {/*没有机会时，游戏结束*/
                context.clearRect(0, 0, 2440, 980);
                alert("GAME OVER!");
            }
            /*碰撞后重置小球的位置*/
            this.bally=500;
            this.ballx=550;
        }
}

}
var n=0;
var inter=setInterval(function () {
    ball.draw();/*绘制小球，*/
    ball.move();/*小球移动，改变速度*/
    ball.checkCanvas();/*检查碰撞*/
},10)




